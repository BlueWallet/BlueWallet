  ...
  async componentDidMount() {
    ...
    try {
      ...
      if (
        ...
        const electrumBalance = await BlueElectrum.getBalanceByAddress(
          addr4elect
        );
        if (electrumBalance.confirmed !== 51432)
          throw new Error(
            "BlueElectrum getBalanceByAddress failure, got " +
              JSON.stringify(electrumBalance)
          );

        const electrumTxs = await BlueElectrum.getTransactionsByAddress(
          addr4elect
        );
      ...
      if (
        ...
        if (hd3.getBalance() !== 26000)
          throw new Error("Could not fetch HD balance");
        await hd3.fetchTransactions();
        if (hd3.transactions.length !== 1)
          throw new Error("Could not fetch HD transactions");

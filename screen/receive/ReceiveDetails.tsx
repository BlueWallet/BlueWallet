...
const ReceiveDetails = () => {
  ...
  const { walletID = wallets.find(w => w.chain === Chain.ONCHAIN)?.getID(), address } = useRoute<RouteProps>().params;
  ...
  const isBIP47Enabled = wallet?.isBIP47Enabled() && wallet !== undefined;
  ...
  const obtainWalletAddress = useCallback(async () => {
    ...
    // If we have an address provided directly, use it regardless of wallet
    if (address) {
    ...
    // Only proceed with wallet-based address generation if wallet exists
    if (!wallet) {
      console.warn('No wallet found and no address provided');
      return;
    }
  ...
  useFocusEffect(
    useCallback(() => {
      const task = InteractionManager.runAfterInteractions(async () => {
        try {
          // Always try to obtain address, regardless of wallet availability

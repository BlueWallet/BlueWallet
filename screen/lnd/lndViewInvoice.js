...
export default class LNDViewInvoice extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      addressText:
        typeof invoice === 'object' && 'payment_request' in invoice
  ...
  async componentDidMount() {
    if (this.state.invoice && this.state.invoice.payment_hash) {
      ...
      let paymentHash = Buffer.from(this.state.invoice.payment_hash).toString(
        'hex'
      )
      ...
      if (this.state.isFetchingInvoices) {
        try {
          ...
          if (typeof updatedUserInvoice !== 'undefined') {
            this.setState({
              invoice: updatedUserInvoice,
              isLoading: false,
              addressText: updatedUserInvoice.payment_request,
            })
      ...
      <SafeBlueArea>
        <ScrollView>
          <View style={styles.activeRoot} onLayout={this.onLayout}>
            ...
            <BlueCopyTextToClipboard
              text={this.state.invoice.payment_request}
            />

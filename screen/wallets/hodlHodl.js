        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'position' : null}>
          <View style={styles.modalContentShort}>
            <FlatList
              style={{ width: '100%' }}
              ItemSeparatorComponent={() => <View style={{ height: 0.5, width: '100%', backgroundColor: '#C8C8C8' }} />}
              data={[{ code: 'currency', native_name: 'Currency' }, { code: 'method', native_name: 'Payment method' }]}
              keyExtractor={(item, index) => item.code}
              renderItem={({ item, index, separators }) => (
                <TouchableHighlight
                  onShowUnderlay={separators.highlight}
        ...
        <BlueCard style={{ alignItems: 'center', flex: 1 }}>
          ...
          {(this.state.isLoading && <BlueLoading />) || (
            <FlatList
              ...
              ListEmptyComponent={() => <Text>No offers. Thy changing country or filters!</Text>}
              renderItem={({ item, index, separators }) => (
                <TouchableHighlight
                  ...
                  <View style={{ backgroundColor: 'white' }}>
                    <View style={{ backgroundColor: 'white', flex: 1, flexDirection: 'row', paddingTop: 20 }}>
                      ...
                      <View style={{ paddingLeft: 10 }}>
                        ...
                        <Text style={{ color: 'grey' }}>{Math.round(item.trader.rating * 100)}%</Text>
                      </View>
                    </View>

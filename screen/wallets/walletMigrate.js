...
export default class WalletMigrate {
  static expoDataDirectory =
    ...
    "/ExponentExperienceData/%40overtorment%2Fbluewallet/RCTAsyncLocalStorage";
  ...
  async migrateDataFromExpo() {
    const expoDirectoryExists = await RNFS.exists(
      RNFS.DocumentDirectoryPath + "/ExponentExperienceData"
    );
    ...
    try {
      await RNFS.copyFile(
        WalletMigrate.expoDataDirectory,
        RNFS.DocumentDirectoryPath + "/RCTAsyncLocalStorage_V1"
      );
    } catch (error) {
      console.log(
    ...
    try {
      await RNFS.unlink(
        RNFS.DocumentDirectoryPath + "/RCTAsyncLocalStorage_V1/.DS_Store"
      );
    } catch (error) {
      console.log(
        "An error was encountered when trying to delete .DS_Store. Continuing migration..."
      );
      console.log(error);
    }
        ...
        if (file.isFile()) {
          if (file.name === "manifest.json") {
            ...
            if (manifestFileParsed.hasOwnProperty("data_encrypted")) {
              if (typeof manifestFileParsed.data_encrypted === "string") {
                await AsyncStorage.setItem(
                  "data_encrypted",
                  manifestFileParsed.data_encrypted
                );
              }
            }
            ...
            if (typeof manifestFileParsed === "object")
              await AsyncStorage.setItem(
                "data",
                JSON.stringify(manifestFileParsed)
              );
          }
        }
      } catch (error) {
    ...
    try {
      ...
      console.log(
        "An error was encountered when trying to delete /ExponentExperienceData. Exiting migration..."
      );
      console.log(error);
    }
    await this.migrateDataToSecureKeystore();
  ...
  async migrateDataToSecureKeystore() {
    try {
      ...
      if (data) {
        const isEncrypted =
          (await AsyncStorage.getItem("data_encrypted")) || "";

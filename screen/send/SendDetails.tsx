...
import { bip21decode, decodeBitcoinUri } from "../../blue_modules/bip21";
...
import {
  ...
  DismissKeyboardInputAccessoryViewID,
} from "../../components/DismissKeyboardInputAccessory";
import HeaderMenuButton from "../../components/HeaderMenuButton";
import InputAccessoryAllFunds, {
  InputAccessoryAllFundsAccessoryViewID,
} from "../../components/InputAccessoryAllFunds";
...
import NetworkTransactionFees, {
  NetworkTransactionFee,
  NetworkTransactionFeeType,
} from "../../models/networkTransactionFees";
import { SendDetailsStackParamList } from "../../navigation/SendDetailsStackParamList";
import {
  CommonToolTipActions,
  ToolTipAction,
} from "../../typings/CommonToolTipActions";
...
const isTXNFile = (filePath: string): boolean => {
  return (
...
const isPossiblySignedPSBTFile = (filePath: string): boolean => {
  return (
    (filePath.toLowerCase().startsWith("file:") ||
      filePath.toLowerCase().startsWith("content:")) &&
...
const isPossiblyPSBTFile = (filePath: string): boolean => {
  return (
    (filePath.toLowerCase().startsWith("file:") ||
      filePath.toLowerCase().startsWith("content:")) &&
...
const SendDetails = () => {
  ...
  const [addresses, setAddresses] = useState<IPaymentDestinations[]>([
    { address: "", key: String(Math.random()), unit: amountUnit },
  ]);
  const [networkTransactionFees, setNetworkTransactionFees] = useState(
    new NetworkTransactionFee(3, 2, 1),
  );
  const [networkTransactionFeesIsLoading, setNetworkTransactionFeesIsLoading] =
    useState(false);
  const [customFee, setCustomFee] = useState<string | null>(null);
  const [selectedPresetFeeRate, setSelectedPresetFeeRate] = useState<
    string | null
  >(null);
  ...
  const feeRate = useMemo(() => {
    ...
    console.log(
      "SendDetails: feeRate useMemo - selectedPresetFeeRate:",
      selectedPresetFeeRate,
    );
    console.log("SendDetails: feeRate useMemo - feePrecalc:", feePrecalc);
    console.log(
      "SendDetails: feeRate useMemo - networkTransactionFees:",
    ...
    if (selectedPresetFeeRate) {
      console.log(
        "SendDetails: Using selectedPresetFeeRate:",
        selectedPresetFeeRate,
      );
    ...
    console.log(
      "SendDetails: No precalc fees yet, using default networkTransactionFees.fastestFee:",
      defaultFee,
    );
    return defaultFee;
    ...
    if (routeParams.uri) {
      try {
        const {
          ...
          memo,
          payjoinUrl: pjUrl,
        } = decodeBitcoinUri(routeParams.uri);
      ...
      setAddresses((prevAddresses) => {
        ...
        if (address) {
          updatedAddresses[index] = {
            ...
            amountSats: amount
              ? btcToSatoshi(amount)
              : updatedAddresses[index].amountSats,
          };
        }
        return updatedAddresses;
      });
  ...
  useEffect(() => {
    ...
    const suitable = wallets.filter(
      (w) => w.chain === Chain.ONCHAIN && w.allowSend(),
    );
    ...
    const newWallet =
      (routeParams.walletID &&
        wallets.find((w) => w.getID() === routeParams.walletID)) ||
      suitable[0];
    setWallet(newWallet);
    setParams({
  ...
  useEffect(() => {
    ...
    if (!utxos) {
      ...
      frozen = wallet
        .getUtxo(true)
        .filter(
          (o) => !lutxo.some((i) => i.txid === o.txid && i.vout === o.vout),
        )
        ...
        try {
          ...
          if (e.message.includes("Not enough") && !flag) {
            ...
            targets = targets.map((t, index) =>
              index > 0 ? { ...t, value: 546 } : { address: t.address },
            );
            continue;
          }
  ...
  const processAddressData = useCallback(
    (data: string | { data?: any }) => {
      ...
      const dataWithoutSchema = data
        ...
        .replace("BITCOIN:", "");
      if (
        wallet.isAddressValid(dataWithoutSchema) ||
        cl.isPaymentCodeValid(dataWithoutSchema)
      ) {
        setAddresses((addrs) => {
      ...
      if (wallet.isAddressValid(address)) {
        setAddresses((addrs) => {
          ...
          addrs[scrollIndex.current].amountSats = new BigNumber(
            options?.amount ?? 0,
          )
  ...
  const createTransaction = async () => {
    ...
    for (const [index, transaction] of addresses.entries()) {
      ...
      if (
        ...
        error =
          frozenBalance > 0
            ? loc.send.details_total_exceeds_balance_frozen
            : loc.send.details_total_exceeds_balance;
        console.log("validation error");
      } else if (transaction.address) {
      ...
      if (!error) {
        const cl = new ContactList();
        if (
          !wallet.isAddressValid(transaction.address) &&
          !cl.isPaymentCodeValid(transaction.address)
        ) {
          console.log("validation error");
          error = loc.send.details_address_field_is_not_valid;
        }
      ...
      if (!error) {
        ...
        if (transaction.address.startsWith("PM")) {
          if (!wallet.allowBIP47()) {
            ...
            !(
              wallet as unknown as AbstractHDElectrumWallet
            ).getBIP47NotificationTransaction(transaction.address)
          ) {
            console.log("validation error");
            ...
            await (
              wallet as unknown as AbstractHDElectrumWallet
            ).syncBip47ReceiversAddresses(transaction.address);
          }
        }
      }
  ...
  const createPsbtTransaction = async () => {
    ...
    const { tx, outputs, psbt, fee } = (
      wallet as HDSegwitBech32Wallet
    )?.createTransaction(
      lutxo,
      targets,
      requestedSatPerByte,
  ...
  const importQrTransaction = useCallback(async () => {
    if (wallet?.type !== WatchOnlyWallet.type) {
      return presentAlert({
        title: loc.errors.error,
        message:
          "Importing transaction in non-watchonly wallet (this should never happen)",
      });
    }
  ...
  const importTransaction = useCallback(async () => {
    ...
    try {
      ...
      if (isPossiblySignedPSBTFile(res.uri)) {
  ...
  const handlePsbtSign = useCallback(
    async (psbtBase64: string) => {
      ...
      for (
        let c = 0;
        c < wallet.next_free_change_address_index + wallet.gap_limit;
        c++
      ) {
        // @ts-ignore hacky
      ...
      navigation.navigate("CreateTransaction", {
        fee: Number(
          new BigNumber(psbt.getFee()).dividedBy(100000000).toNumber(),
        ),
  ...
  useEffect(() => {
    ...
    if (data) {
      if (selectedDataProcessor.current) {
        console.debug(
          "SendDetails - selectedDataProcessor:",
          selectedDataProcessor.current,
        );
  ...
  const handleAddRecipient = useCallback(() => {
    ...
    const incompleteIndex = addresses.findIndex(
      (item) => !item.address || !item.amount,
    );
  ...
  const handleRemoveAllRecipients = useCallback(() => {
    Alert.alert(
      loc.send.details_recipients_title,
      loc.send.details_add_recc_rem_all_alert_description,
      [
        {
          text: loc._.cancel,
          onPress: () => {},
  ...
  const handleRemoveRecipient = useCallback(() => {
    if (addresses.length > 1) {
      ...
      const newIndex =
        scrollIndex.current >= newAddresses.length
  ...
  const onUseAllPressed = useCallback(() => {
    ...
    const message =
      frozenBalance > 0
        ? loc.send.details_adv_full_sure_frozen
  ...
  const headerRightActions = useCallback(() => {
    ...
    const isSendMaxUsed = addresses.some(
      (element) => element.amount === BitcoinUnit.MAX,
    );
    ...
    const transactionActions: Action[] = [
      ...
      },
      {
        ...
        hidden: !(
          ...
          wallet.howManySignaturesCanWeMake() > 0
        ),
      },
      {
  ...
  useEffect(() => {
    ...
    console.log(
      "SendDetails: current selectedPresetFeeRate:",
      selectedPresetFeeRate,
    );
    ...
    if (selectedFeeRate !== undefined || selectedFeeType !== undefined) {
      ...
      if (selectedFeeType === NetworkTransactionFeeType.CUSTOM) {
        console.log(
          "SendDetails: CUSTOM fee selected, setting customFee to:",
          selectedFeeRate,
        );
        ...
        console.log(
          "SendDetails: Setting selectedPresetFeeRate to:",
          selectedFeeRate,
        );
    ...
    routeParams.selectedFeeRate,
    routeParams.selectedFeeType,
    networkTransactionFees,
    setParams,
    customFee,
    selectedPresetFeeRate,
  ...
  const calculateTotalAmount = () => {
    const totalAmount = addresses.reduce(
      (total, item) => total + Number(item.amountSats || 0),
      0,
    );
    ...
    return (
      <View style={styles.select}>
        ...
        <View style={styles.selectWrap}>
          ...
          >
            <Text style={[styles.selectLabel, stylesHook.selectLabel]}>
              {wallet?.getLabel()}
            </Text>
          </TouchableOpacity>
        </View>
  ...
  const renderBitcoinTransactionInfoFields = (params: {
    item: IPaymentDestinations;
    index: number;
  }) => {
    const { item, index } = params;
    return (
      <View
        style={[styles.transactionItemContainer, { width: dimensions.width }]}
              ...
              setAddresses((addrs) => {
                ...
                switch (unit) {
                  ...
                  case BitcoinUnit.LOCAL_CURRENCY:
                    ...
                    addr.amountSats =
                      AmountInput.getCachedSatoshis(String(addr.amount)) ||
                      btcToSatoshi(fiatToBTC(Number(addr.amount)));
                    break;
                }
              ...
              setAddresses((addrs) => {
                ...
                switch (item.unit || amountUnit) {
                  case BitcoinUnit.BTC:
                    ...
                    break;
                  case BitcoinUnit.LOCAL_CURRENCY:
                    item.amountSats = btcToSatoshi(
                      fiatToBTC(Number(item.amount)),
                    );
                    break;
                  case BitcoinUnit.SATS:
                  default:
    ...
    <SafeArea style={[styles.root, stylesHook.root]}>
      <View>
        ...
        <TouchableOpacity
          ...
          style={styles.fee}
        >
          <Text style={[styles.feeLabel, stylesHook.feeLabel]}>
            {loc.send.create_fee}
          </Text>
      ...
      {Platform.select({
        ios: (
          <InputAccessoryAllFunds
            canUseAll={balance > 0}
            onUseAllPressed={onUseAllPressed}
            balance={String(allBalance)}
          />
        ),
        android: isVisible && (
          <InputAccessoryAllFunds

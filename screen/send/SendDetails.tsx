...
const SendDetails = () => {
  ...
  const handlePsbtSign = useCallback(
    async (psbtBase64: string) => {
      ...
      // @ts-ignore hacky
      for (let c = 0; c < (wallet as any).next_free_change_address_index + (wallet as any).gap_limit; c++) {
        // @ts-ignore hacky
        changeAddresses.push((wallet as any)._getInternalAddressByIndex(c));
      }
    ...
    return (
      <View style={[styles.transactionItemContainer, { width: dimensions.width }]} testID={'Transaction' + index}>
        ...
        <View style={styles.addressInputContainer}>
          <AddressInput
            onChangeText={text => {
              const { address, amount, memo, payjoinUrl: pjUrl } = decodeBitcoinUri(text.trim());
              setAddresses(addrs => {

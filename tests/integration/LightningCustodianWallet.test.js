...
describe("LightningCustodianWallet", () => {
  ...
  it("can use existing login/pass", async () => {
    ...
    for (const tx of l2.getTransactions()) {
      ...
      assert.ok(
        tx.type === "bitcoind_tx" || tx.type === "paid_invoice",
        "unexpected tx type " + tx.type
      );
    }
  ...
  it("can decode & check invoice", async () => {
    ...
    const decoded = await l2.decodeInvoice(invoice);
  ...
  it("can pay invoice", async () => {
    ...
    assert.strictEqual(
      typeof lastTx.payment_preimage,
      "string",
      "preimage is present and is a string"
    );
    assert.strictEqual(
      lastTx.payment_preimage.length,
      64,
      "preimage is present and is a string of 32 hex-encoded bytes"
    );
  ...
  it("can create invoice and pay other blitzhub invoice", async () => {
    ...
    await lOld.fetchTransactions();
    assert.strictEqual(
      lOld.transactions_raw.length,
      txLen + 1,
      "internal invoice should also produce record in payer`s tx list"
    );
    ...
    assert.strictEqual(
      txLen,
      lOld.transactions_raw.length,
      "tx count should not be changed"
    );
    assert.strictEqual(
      invLen,
      (await lNew.getUserInvoices()).length,
      "invoices count should not be changed"
    );

...
describe('import procedure', () => {
  ...
  it('can import lightning spark wallet', async () => {
    const store = createStore();
    const { promise } = startImport(
      'spark://abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about',
      false,
      false,
      false,
      ...store.callbacks,
    ...
    assert.strictEqual(store.state.wallets.length, 1);
    assert.strictEqual(store.state.wallets[0].type, LightningSparkWallet.type);
    assert.strictEqual(
      store.state.wallets[0].getSecret(),
      'spark://abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about',
    );

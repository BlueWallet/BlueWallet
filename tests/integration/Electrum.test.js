...
describe("Electrum", () => {
  it("ElectrumClient can test connection", async () => {
    assert.ok(
      await BlueElectrum.testConnection("electrum1.bluewallet.io", "50001")
    );
  });
  ...
  it("BlueElectrum can do getTransactionsByAddress()", async function() {
    const txs = await BlueElectrum.getTransactionsByAddress(
  ...
  it("BlueElectrum can do getTransactionsFullByAddress()", async function() {
    const txs = await BlueElectrum.getTransactionsFullByAddress(
      "bc1qt4t9xl2gmjvxgmp5gev6m8e6s9c85979ta7jeh"
    );
    for (const tx of txs) {

...
describe('import procedure', () => {
  ...
  it('can be stopped', async () => {
    const store = createStore();
    const { promise, stop } = startImport('KztVRmc2EJJBHi599mCdXrxMTsNsGy3NUjc3Fb3FFDSMYyMDRjnv', store.callbacks);
    stop();
  ...
  it('can import Legacy', async () => {
    const store = createStore();
    const { promise } = startImport('KztVRmc2EJJBHi599mCdXrxMTsNsGy3NUjc3Fb3FFDSMYyMDRjnv', store.callbacks);
    await promise;
  ...
  it('can import Legacy P2SH Segwit', async () => {
    const store = createStore();
    const { promise } = startImport('L3NxFnYoBGjJ5PhxrxV6jorvjnc8cerYJx71vXU6ta8BXQxHVZya', store.callbacks);
    await promise;
  ...
  it('can import Legacy Bech32 Segwit', async () => {
    const store = createStore();
    const { promise } = startImport('L1T6FfKpKHi8JE6eBKrsXkenw34d5FfFzJUZ6dLs2utxkSvsDfxZ', store.callbacks);
    await promise;
  ...
  it('can import HD Legacy Electrum (BIP32 P2PKH)', async () => {
    const store = createStore();
    const { promise } = startImport('eight derive blast guide smoke piece coral burden lottery flower tomato flame', store.callbacks);
    await promise;
  ...
  it('can import HD Legacy Electrum (BIP32 P2PKH) with passphrase', async () => {
    ...
    const { promise } = startImport('receive happy wash prosper update pet neck acid try profit proud hungry', store.callbacks);
    await promise;
    assert.strictEqual(store.state.wallets[0].type, HDLegacyElectrumSeedP2PKHWallet.type);
  ...
  it('can import BreadWallet', async () => {
    const store = createStore();
    const { promise } = startImport('become salmon motor battle sweet merit romance ecology age squirrel oblige awesome', store.callbacks);
    await promise;
  ...
  it('can import HD Electrum (BIP32 P2WPKH)', async () => {
    const store = createStore();
    const { promise } = startImport('noble mimic pipe merry knife screen enter dune crop bonus slice card', store.callbacks);
    await promise;
  ...
  it('can import HD Electrum (BIP32 P2WPKH) with passphrase', async () => {
    ...
    const { promise } = startImport('bitter grass shiver impose acquire brush forget axis eager alone wine silver', store.callbacks);
    await promise;
    assert.strictEqual(store.state.wallets[0].type, HDSegwitElectrumSeedP2WPKHWallet.type);
  ...
  it('importing empty BIP39 should yield BIP84', async () => {
    ...
    const { promise } = startImport(tempWallet.getSecret(), store.callbacks);
    await promise;
    assert.strictEqual(store.state.wallets[0].type, HDSegwitBech32Wallet.type);
  });
  ...
  it('can import Legacy with uncompressed pubkey', async () => {
    const store = createStore();
    const { promise } = startImport('5KE6tf9vhYkzYSbgEL6M7xvkY69GMFHF3WxzYaCFMvwMxn3QgRS', store.callbacks);
    await promise;
  ...
  it('can import BIP38 encrypted backup', async () => {
    ...
    const { promise } = startImport('6PnU5voARjBBykwSddwCdcn6Eu9EcsK24Gs5zWxbJbPZYW7eiYQP8XgKbN', store.callbacks);
    await promise;
  ...
  it('can import watch-only address', async () => {
    ...
    const { promise: promise1 } = startImport('1AhcdMCzby4VXgqrexuMfh7eiSprRFtN78', store1.callbacks);
    await promise1;
    assert.strictEqual(store1.state.wallets[0].type, WatchOnlyWallet.type);

    const store2 = createStore();
    const { promise: promise2 } = startImport('3EoqYYp7hQSHn5nHqRtWzkgqmK3caQ2SUu', store2.callbacks);
    await promise2;
    assert.strictEqual(store2.state.wallets[0].type, WatchOnlyWallet.type);

    const store3 = createStore();
    const { promise: promise3 } = startImport('bc1q8j4lk4qlhun0n7h5ahfslfldc8zhlxgynfpdj2', store3.callbacks);
    await promise3;

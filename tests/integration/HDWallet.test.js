...
it("can convert witness to address", () => {
  ...
  address = SegwitP2SHWallet.scriptPubKeyToAddress(
    "a914e286d58e53f9247a4710e51232cce0686f16873c87"
  );
  ...
  address = SegwitBech32Wallet.scriptPubKeyToAddress(
    "00144d757460da5fcaf84cc22f3847faaa1078e84f6a"
  );
...
it("can create a Segwit HD (BIP49)", async function() {
  ...
  assert.strictEqual(
    "35p5LwCAE7mH2css7onyQ1VuS1jgWtQ4U3",
    hd._getExternalAddressByIndex(1)
  );
  assert.strictEqual(
    "32yn5CdevZQLk3ckuZuA8fEKBco8mEkLei",
  ...
  assert.strictEqual(
    "L4MqtwJm6hkbACLG4ho5DF8GhcXdLEbbvpJnbzA9abfD6RDpbr2m",
...
it("Segwit HD (BIP49) can fetch UTXO", async function() {
  ...
  hd.usedAddresses = [
    ...
    "1BiTCHeYzJNMxBLFCMkwYXNdFEdPJP53ZV"
  ]; // hacking internals
  await hd.fetchUtxo();
...
it("Segwit HD (BIP49) can fetch balance with many used addresses in hierarchy", async function() {
  ...
  took > 15 &&
    console.warn("took", took, "sec to fetch huge HD wallet's balance");
...
it("can create a Legacy HD (BIP44)", async function() {
  ...
  assert.strictEqual(
    hd._getExternalAddressByIndex(1),
    "1QDCFcpnrZ4yrAQxmbvSgeUC9iZZ8ehcR5"
  );
  assert.strictEqual(
    hd._getInternalAddressByIndex(0),
    "1KZjqYHm7a1DjhjcdcjfQvYfF2h6PqatjX"
  );
  assert.strictEqual(
    hd._getInternalAddressByIndex(1),
    "13CW9WWBsWpDUvLtbFqYziWBWTYUoQb4nU"
  );
  assert.strictEqual(
    hd.getXpub(),
  ...
  assert.strictEqual(
    hd._getExternalWIFByIndex(0),
    "L1hqNoJ26YuCdujMBJfWBNfgf4Jo7AcKFvcNcKLoMtoJDdDtRq7Q"
  );
  assert.strictEqual(
    hd._getExternalWIFByIndex(1),
    "KyyH4h59iatJWwFfiYPnYkw39SP7cBwydC3xzszsBBXHpfwz9cKb"
  );
  assert.strictEqual(
    hd._getInternalWIFByIndex(0),
    "Kx3QkrfemEEV49Mj5oWfb4bsWymboPdstta7eN3kAzop9apxYEFP"
  );
  assert.strictEqual(
    hd._getInternalWIFByIndex(1),
    "Kwfg1EDjFapN9hgwafdNPEH22z3vkd4gtG785vXXjJ6uvVWAJGtr"
  );
  await hd.fetchBalance();
...
it("HD breadwallet works", async function() {
  ...
  assert.strictEqual(
    hdBread._getExternalAddressByIndex(hdBread.next_free_address_index),
    freeAddress
  );
});

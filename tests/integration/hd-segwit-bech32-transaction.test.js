...
describe("HDSegwitBech32Transaction", () => {
  ...
  it("can tell if its our transaction", async function() {
    ...
    tt = new HDSegwitBech32Transaction(
      null,
      "89bcff166c39b3831e03257d4bcc1034dd52c18af46a3eb459e72e692a88a2d8",
      hd
    );
      ...
      JSON.stringify([
        {
          ...
          value: 108150,
          txId:
            "f3d7fb23248168c977e8085b6bd5381d73c85da423056a47cbf734b5665615f1",
  ...
  it("can do RBF - cancel tx", async function() {
    ...
    const addr = SegwitBech32Wallet.scriptPubKeyToAddress(
      createdTx.outs[0].script
    );
    assert.ok(hd.weOwnAddress(addr));

    const actualFeerate =
      (108150 + 200000 - createdTx.outs[0].value) / (tx.toHex().length / 2);
  ...
  it("can do RBF - bumpfees tx", async function() {
    ...
    const addr0 = SegwitP2SHWallet.scriptPubKeyToAddress(
      createdTx.outs[0].script
    );
    assert.ok(!hd.weOwnAddress(addr0));
    assert.strictEqual(addr0, "3NLnALo49CFEF4tCRhCvz45ySSfz3UktZC"); // dest address
    const addr1 = SegwitBech32Wallet.scriptPubKeyToAddress(
      createdTx.outs[1].script
    );
    assert.ok(hd.weOwnAddress(addr1));

    const actualFeerate =
      (108150 + 200000 - (createdTx.outs[0].value + createdTx.outs[1].value)) /
      (tx.toHex().length / 2);
  ...
  it("can do CPFP - bump fees", async function() {
    ...
    const avgFeeRate =
      (oldFee + fee) / (tt._txhex.length / 2 + tx.toHex().length / 2);

...
describe("Bech32 Segwit HD (BIP84)", () => {
  it("can create", async function() {
    ...
    assert.ok(
      hd._getInternalAddressByIndex(0) !== hd._getInternalAddressByIndex(1)
    );

    assert.strictEqual(
      hd._getDerivationPathByAddress(hd._getExternalAddressByIndex(0)),
      "m/84'/0'/0'/0/0"
    );
    assert.strictEqual(
      hd._getDerivationPathByAddress(hd._getExternalAddressByIndex(1)),
      "m/84'/0'/0'/0/1"
    );
    assert.strictEqual(
      hd._getDerivationPathByAddress(hd._getInternalAddressByIndex(0)),
      "m/84'/0'/0'/1/0"
    );
    assert.strictEqual(
      hd._getDerivationPathByAddress(hd._getInternalAddressByIndex(1)),
      "m/84'/0'/0'/1/1"
    );
    ...
    assert.strictEqual(
      hd._getExternalAddressByIndex(hd.next_free_address_index),
      freeAddress
    );
  ...
  it("can fetch balance", async function() {
    ...
    assert.strictEqual(
      hd._getInternalAddressByIndex(1),
    ...
    assert.strictEqual(
      await hd.getAddressAsync(),
      hd._getExternalAddressByIndex(2)
    );
    assert.strictEqual(
      await hd.getChangeAddressAsync(),
      hd._getInternalAddressByIndex(2)
    );
  ...
  it("can generate", async () => {
    ...
    for (let c = 0; c < 1000; c++) {
      ...
      hashmap[secret] = 1;
      assert.ok(
        secret.split(" ").length === 12 || secret.split(" ").length === 24
      );
    }
  ...
  it("can fetchBalance, fetchTransactions, fetchUtxo and create transactions", async () => {
    ...
    let end = +new Date();
      ...
      console.warn("fetchBalance took", (end - start) / 1000, "sec");
    ...
    await hd.fetchTransactions();
      ...
      console.warn("fetchTransactions took", (end - start) / 1000, "sec");

    start = +new Date();
    await hd.fetchBalance();
    end = +new Date();
    end - start > 2000 &&
      console.warn("warm fetchBalance took", (end - start) / 1000, "sec");
    ...
    await hd.fetchTransactions();
      ...
      console.warn("warm fetchTransactions took", (end - start) / 1000, "sec");

    let txFound = 0;
    for (const tx of hd.getTransactions()) {
      if (
        tx.hash ===
        "e9ef58baf4cff3ad55913a360c2fa1fd124309c59dcd720cdb172ce46582097b"
      ) {
        assert.strictEqual(tx.value, -129545);
        assert.strictEqual(
          tx.inputs[0].addresses[0],
          "bc1qffcl35r05wyf06meu3dalfevawx559n0ufrxcw"
        );
        assert.strictEqual(
          tx.inputs[1].addresses[0],
          "bc1qtvh8mjcfdg9224nx4wu3sw7fmmtmy2k3jhdeul"
        );
        assert.strictEqual(
          tx.inputs[2].addresses[0],
          "bc1qhe03zgvq4fmfw8l2qq2zu4dxyhgyukcz6k2a5w"
        );
        txFound++;
      }
      if (
        tx.hash ===
        "e112771fd43962abfe4e4623bf788d6d95ff1bd0f9b56a6a41fb9ed4dacc75f1"
      ) {
        assert.strictEqual(tx.value, 1000000);
        assert.strictEqual(
          tx.inputs[0].addresses[0],
          "3NLnALo49CFEF4tCRhCvz45ySSfz3UktZC"
        );
        assert.strictEqual(
          tx.inputs[1].addresses[0],
          "3NLnALo49CFEF4tCRhCvz45ySSfz3UktZC"
      ...
      if (
        tx.hash ===
        "c94bdec21c72d3441245caa164b00315b131f6b72513369f4be1b00b9fb99cc5"
      ) {
        assert.strictEqual(
          tx.inputs[0].addresses[0],
          "16Nf5X77RbFz9Mb6t2GFqxs3twQN1joBkD"
        );
        txFound++;
      }
      if (
        tx.hash ===
        "51fc225ddf24f7e124f034637f46442645ca7ea2c442b28124d4bcdd04e30195"
      ) {
        assert.strictEqual(
          tx.inputs[0].addresses[0],
          "3NLnALo49CFEF4tCRhCvz45ySSfz3UktZC"

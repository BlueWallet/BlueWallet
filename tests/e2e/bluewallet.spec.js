...
import multisigSigning from '../fixtures/multisig-signing.json';
...
describe('BlueWallet UI Tests - no wallets', () => {
  ...
  it('can import multisig setup from UR and sign an imported transaction on hw devices', async () => {
    ...
    const urs = multisigSigning.setupUr;
    ...
    // The public fixture wallet's original outputs have been spent. Import the
    // historical unsigned PSBT that matches the recorded hardware signatures;
    // this flow verifies signing and never broadcasts the transaction.
    await tapHeaderMenuItem('Import Transaction (QR)', {
      actionId: 'import_transaction_qr',
      restoreSynchronization: false,
    });
    await scanText(multisigSigning.unsignedPsbt);

    await waitFor(element(by.id('ItemUnsigned')).atIndex(0))
      .toBeVisible()
      .withTimeout(30_000);
    await expect(element(by.id('ItemSigned'))).not.toBeVisible();
    ...
    const ursSignedByPassport = multisigSigning.passportUr;
    ...
    try {
      ...
      const urSignedByPassportAndKeystone = multisigSigning.fullySignedUr;

...
describe('shouldOfferBip39Suggestions', () => {
  ...
  it('allows a single completed typo while the next word is being typed', () => {
    assert.strictEqual(shouldOfferBip39Suggestions('abandone aban'), true);
  });

  it('rejects after two completed words outside the english bip39 wordlist', () => {
    assert.strictEqual(shouldOfferBip39Suggestions('casa perro aban'), false);
  });
...
describe('getImportWalletSuggestions', () => {
  ...
  it('returns matches after a single completed typo', () => {
    const matches = getImportWalletSuggestions('abandone aban', 13);
    assert.ok(matches.includes('abandon'));
  });

  it('returns empty array after two completed non-english words', () => {
    assert.deepStrictEqual(getImportWalletSuggestions('casa perro aban', 15), []);
  });

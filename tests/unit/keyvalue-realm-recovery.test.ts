...
describe('keyvalue realm backup recovery', () => {
  ...
  it('shares first-time key initialization across concurrent storage instances', async () => {
    const readers = Array.from({ length: 8 }, () => new BlueApp());
    const realms = await Promise.all(readers.map(storage => storage.openRealmKeyValue()));
    const credentials = await Keychain.getGenericPassword({ service: 'realm_encryption_key' });

    expect(Keychain.setGenericPassword).toHaveBeenCalledTimes(1);
    expect(Realm.open).toHaveBeenCalledTimes(readers.length);
    for (const [config] of Realm.open.mock.calls) {
      expect(uint8ArrayToHex(new Uint8Array(config.encryptionKey))).toBe(credentials.password);
    }
    realms.forEach(realm => realm.close());
  });

  it('retries key initialization after Keychain fails without opening Realm', async () => {
    Keychain.setGenericPassword.mockResolvedValueOnce(false);
    const storage = new BlueApp();

    await expect(storage.openRealmKeyValue()).rejects.toThrow('Unable to store Realm encryption key');
    expect(Realm.open).not.toHaveBeenCalled();
    const realm = await storage.openRealmKeyValue();
    expect(Keychain.setGenericPassword).toHaveBeenCalledTimes(2);
    realm.close();
  });

  it('reads a replacement Keychain key after initialization rather than caching the old key', async () => {
    const storage = new BlueApp();
    (await storage.openRealmKeyValue()).close();
    const replacement = 'ab'.repeat(64);
    await Keychain.setGenericPassword('realm_encryption_key', replacement, { service: 'realm_encryption_key' });

    (await storage.openRealmKeyValue()).close();

    const config = Realm.open.mock.calls[Realm.open.mock.calls.length - 1][0];
    expect(uint8ArrayToHex(new Uint8Array(config.encryptionKey))).toBe(replacement);
  });

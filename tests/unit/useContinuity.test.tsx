...
describe('useContinuity', () => {
  ...
  it('treats boolean false and numeric zero as meaningful userInfo values', () => {
    render(<HookRunner title="SendDetails flags" type={ContinuityActivityType.SendOnchain} userInfo={{ amount: 0, isBatch: false }} />);

    assert.strictEqual(mockBecomeCurrent.mock.calls.length, 1);
    assert.deepStrictEqual(mockBecomeCurrent.mock.calls[0][3], { amount: 0, isBatch: false });
  });

  it('calls becomeCurrent when nested array/object has at least one meaningful value', () => {
    render(
      <HookRunner
        title="SendDetails nested"
        type={ContinuityActivityType.SendOnchain}
        userInfo={{
          recipients: [
            { address: '', amount: '' },
            { address: 'bc1qnested', amount: '' },
          ],
        }}
      />,
    );
  ...
  it('invalidates existing activity when params become empty and does not create a new one', () => {
    const { rerender } = render(
      <HookRunner
        title="View Transaction"
        type={ContinuityActivityType.ViewInBlockExplorer}
        url="https://mempool.space/tx/abc123"
        userInfo={{ address: 'bc1qtest' }}
      />,
    );

    const firstId = mockBecomeCurrent.mock.calls[0][0];

    rerender(<HookRunner title="View Transaction" type={ContinuityActivityType.ViewInBlockExplorer} url="   " userInfo={{}} />);

...
const SelectFeeModal = forwardRef<BottomModalHandle, SelectFeeModalProps>(
  ({ networkTransactionFees, feePrecalc, feeRate, setCustomFee, setFeePrecalc }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      present: () => feeModalRef.current?.present(),
      dismiss: () => feeModalRef.current?.dismiss(),
    }));
    ...
    return (
      <BottomModal
        ...
        footer={
          <View style={styles.feeModalFooter}>
            <TouchableOpacity testID="feeCustom" accessibilityRole="button" onPress={() => customModalRef.current?.present()}>
      ...
      >
        ...
        <BottomModal
          ...
          backgroundColor={colors.modal}
          contentContainerStyle={[styles.modalContent, styles.modalContentMinHeight]}
          footer={  <View style={styles.feeModalFooter}>
          <Button title={loc._.cancel} type="secondary" onPress={() => customModalRef.current?.dismiss()} />
          <Button titl={loc._.ok} onPress={handleCustomFeeSubmit} />
        </View>}
            footerDefaultMargins
        >
          <View style={styles.paddingTop80}>

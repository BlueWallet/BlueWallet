...
} from "../HandOffComponent";
import { useStorage } from "../../hooks/context/useStorage";
import { BitcoinUnit } from "../../models/bitcoinUnits";
import {
  TotalWalletsBalanceKey,
  TotalWalletsBalancePreferredUnit,
...
export const setTotalBalanceViewEnabledStorage = async (
  value: boolean,
): Promise<void> => {
  try {
    await DefaultPreference.setName(GROUP_IO_BLUEWALLET);
...
export const getIsTotalBalanceViewEnabled = async (): Promise<boolean> => {
  try {
    ...
    const isEnabledValue =
      (await DefaultPreference.get(TotalWalletsBalanceKey)) ?? "true";
...
export const setTotalBalancePreferredUnitStorageFunc = async (
  unit: BitcoinUnit,
): Promise<void> => {
  try {
    await DefaultPreference.setName(GROUP_IO_BLUEWALLET);
...
export const getTotalBalancePreferredUnit = async (): Promise<BitcoinUnit> => {
  try {
    await DefaultPreference.setName(GROUP_IO_BLUEWALLET);
    const unit = (await DefaultPreference.get(
      TotalWalletsBalancePreferredUnit,
    )) as BitcoinUnit | null;
...
export const SettingsProvider: React.FC<{ children: React.ReactNode }> =
  React.memo(({ children }: { children: React.ReactNode }) => {
    const [preferredFiatCurrency, setPreferredFiatCurrencyState] =
      useState<TFiatUnit>(FiatUnit.USD);
    const [language, setLanguage] = useState<string>("en");
    const [isHandOffUseEnabled, setIsHandOffUseEnabledState] =
      useState<boolean>(false);
    const [isPrivacyBlurEnabled, setIsPrivacyBlurEnabled] =
      useState<boolean>(true);
    const [isDoNotTrackEnabled, setIsDoNotTrackEnabled] =
      useState<boolean>(false);
    const [isWidgetBalanceDisplayAllowed, setIsWidgetBalanceDisplayAllowed] =
      useState<boolean>(true);
    const [isDynamicIslandEnabled, setIsDynamicIslandEnabled] =
      useState<boolean>(true);
    const [isLegacyURv1Enabled, setIsLegacyURv1Enabled] =
      useState<boolean>(false);
    const [isClipboardGetContentEnabled, setIsClipboardGetContentEnabled] =
      useState<boolean>(true);
    const [isQuickActionsEnabled, setIsQuickActionsEnabled] =
      useState<boolean>(true);
    const [isTotalBalanceEnabled, setIsTotalBalanceEnabled] =
      useState<boolean>(true);
    const [totalBalancePreferredUnit, setTotalBalancePreferredUnit] =
      useState<BitcoinUnit>(BitcoinUnit.BTC);
    const [selectedBlockExplorer, setSelectedBlockExplorer] =
      useState<BlockExplorer>(BLOCK_EXPLORERS.default);
        ...
        const promises: Promise<void>[] = [
          ...
          getBlockExplorerUrl().then((url) => {
            const predefinedExplorer = Object.values(BLOCK_EXPLORERS).find(
              (explorer) => normalizeUrl(explorer.url) === normalizeUrl(url),
            );
            setSelectedBlockExplorer(
              predefinedExplorer ??
                ({ key: "custom", name: "Custom", url } as BlockExplorer),
            );
          }),
        ];
    ...
    useEffect(() => {
      initCurrencyDaemon()
        ...
        .catch((e) => {
          console.error(
            "Error initializing currency daemon or getting preferred currency:",
            e,
          );
        });
    }, []);
    ...
    const setPreferredFiatCurrencyStorage = useCallback(
      async (currency: TFiatUnit): Promise<void> => {
        try {
    ...
    const setIsWidgetBalanceDisplayAllowedStorage = useCallback(
      async (value: boolean): Promise<void> => {
        try {
    ...
    const setIsDynamicIslandEnabledStorage = useCallback(
      async (value: boolean): Promise<void> => {
        try {
    ...
    const setIsLegacyURv1EnabledStorage = useCallback(
      async (value: boolean): Promise<void> => {
        try {
          if (value) {
            await setUseURv1();
    ...
    const setIsClipboardGetContentEnabledStorage = useCallback(
      async (value: boolean): Promise<void> => {
        try {
    ...
    const setIsQuickActionsEnabledStorage = useCallback(
      async (value: boolean): Promise<void> => {
        try {
    ...
    const setIsTotalBalanceEnabledStorage = useCallback(
      async (value: boolean): Promise<void> => {
        try {
    ...
    const setTotalBalancePreferredUnitStorage = useCallback(
      async (unit: BitcoinUnit): Promise<void> => {
        try {
    ...
    const setBlockExplorerStorage = useCallback(
      async (explorer: BlockExplorer): Promise<boolean> => {
        try {

...
object MarketAPI {
    ...
    fun fetchPrice(context: Context, currency: String): String? {
        try {
            ...
            return try {
                ...
                parseJSONBasedOnSource(responseJson, source, endPointKey)
            } finally {
    ...
    private fun buildURLString(source: String, endPointKey: String): String {
        return when (source) {
            ...
            "coinpaprika" -> "https://api.coinpaprika.com/v1/tickers/btc-bitcoin?quotes=$endPointKey"
    ...
    private fun parseJSONBasedOnSource(json: JSONObject, source: String, endPointKey: String): String? {
        return try {
            when (source) {
                ...
                "coinpaprika" -> json.getJSONObject("quotes").getJSONObject(endPointKey.uppercase()).getString("price")
                "Exir" -> json.getString("last")
                "Yadio", "YadioConvert" -> json.getJSONObject(endPointKey).getString("price")
                "CoinDesk" -> json.getJSONObject("bpi").getJSONObject(endPointKey.uppercase()).getString("rate_float")
                else -> throw IllegalArgumentException("Unsupported source: $source")
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error parsing JSON for source $source", e)
            null
        }
    }
}
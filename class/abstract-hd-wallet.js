...
export class AbstractHDWallet extends LegacyWallet {
  ...
  static convertTx(tx) {
    ...
    for (const o of clone.outputs) {
      o.addresses = [o.addr];
    }
    for (const i of clone.inputs) {
  ...
  async fetchBalance() {
    try {
      ...
      async function binarySearchIterationForInternalAddress(index, maxUsedIndex = 0, minUnusedIndex = 100500100, depth = 0) {
        ...
        const txs = await BlueElectrum.getTransactionsByAddress(that._getInternalAddressByIndex(index));
        if (txs.length === 0) {
          if (index === 0) return 0;
          ...
          const txs2 = await BlueElectrum.getTransactionsByAddress(that._getInternalAddressByIndex(index + 1));
      ...
      async function binarySearchIterationForExternalAddress(index, maxUsedIndex = 0, minUnusedIndex = 100500100, depth = 0) {
        if (depth >= 20) return maxUsedIndex + 1; // fail
        const txs = await BlueElectrum.getTransactionsByAddress(that._getExternalAddressByIndex(index));
        if (txs.length === 0) {
          if (index === 0) return 0;
          minUnusedIndex = Math.min(minUnusedIndex, index); // set
          index = Math.floor((index - maxUsedIndex) / 2 + maxUsedIndex);
          ...
          maxUsedIndex = Math.max(maxUsedIndex, index); // set
          const txs2 = await BlueElectrum.getTransactionsByAddress(that._getExternalAddressByIndex(index + 1));
          if (txs2.length === 0) return index + 1; // thats our next free address
      ...
      if (this.next_free_change_address_index === 0 && this.next_free_address_index === 0) {
        ...
        const txs = await BlueElectrum.getTransactionsByAddress(that._getInternalAddressByIndex(0));
        if (txs.length === 0) {
          const txs2 = await BlueElectrum.getTransactionsByAddress(that._getExternalAddressByIndex(0));
          if (txs2.length === 0) {
  ...
  async _fetchBalance() {
    ...
    const balance = await BlueElectrum.multiGetBalanceByAddress(this.usedAddresses);
  ...
  async fetchUtxo() {
    ...
    this.utxo = [];
    const addresses = this.usedAddresses;
    ...
    for (const addr of addresses) {
      batch.push(addr);
      if (batch.length >= 75) {
        const utxos = await this._fetchUtxoBatch(batch);

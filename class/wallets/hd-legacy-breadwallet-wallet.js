...
export class HDLegacyBreadwalletWallet extends HDLegacyP2PKHWallet {
  ...
  _calcNodeAddressByIndex(node, index, p2wpkh = false) {
    let _node;
    if (node === 0) {
      _node = this._node0 || (this._node0 = bitcoinjs.bip32.fromBase58(this.getXpub(), network).derive(node));
    }
    if (node === 1) {
      _node = this._node1 || (this._node1 = bitcoinjs.bip32.fromBase58(this.getXpub(), network).derive(node));
    }
